...
BorderedMessage.prototype.toString = function () {
    ...
    if (!chalk[config.borderColor] || !chalk[config.textColor]) {
    ...
    for (var i = 0; i < data.length; i++) {
        message += sideSeparator + chalk[config.textColor](data[i].text) + pmUtil.fillString(' ', data[i].rest) + sideSeparator + '\n'
    }
